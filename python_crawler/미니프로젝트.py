...
SMTP_PORT = 465
SMTP_USER = myGmailAccount.user  # 개인 정보 myGmailAccout.py에 user, pw변수 만든 후 gitignore에 추가
SMTP_PASSWORD = myGmailAccount.pw
...
for r in ws.iter_rows(min_row=3):
    if str(r[0].value) == 'None' or str(r[2].value) == 'None':
        break
    email_to_list.append([r[1].value, r[2].value])

news_wb = load_workbook(excel_name)
news_ws = news_wb.active
contents = ""

for r in news_ws.iter_rows(min_row=2):
    for content in r:
        contents += str(content.value) + " "
    contents += "\n"

for name, email in email_to_list:
    send_mail(name, email, input_keyword + "관련 속보입니다.\n", contents, excel_name)
